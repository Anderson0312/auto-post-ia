...
const videoEnabled = process.env.NEXT_PUBLIC_VIDEO_GENERATION_ENABLED === "true"
    ...
    <div className="min-h-screen bg-gray-50">
      <header className="bg-white border-b">
        <div className="max-w-6xl mx-auto px-4 py-4 flex items-center justify-between">
          <div>
            <h1 className="text-2xl font-bold">{project?.title || "Projeto"}</h1>
      ...
      <main className="max-w-6xl mx-auto px-4 py-6 space-y-6">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <Button onClick={() => runAction("script")} disabled={!!actionLoading}>
            {actionLoading === "script" ? "Gerando..." : "1. Gerar Roteiro"}
          </Button>
          <Button onClick={() => runAction("storyboard")} disabled={!!actionLoading}>
            {actionLoading === "storyboard" ? "Gerando..." : "2. Gerar Storyboard"}
          </Button>
          {videoEnabled ? (
            <Button onClick={() => runAction("video")} disabled={!!actionLoading}>
              {actionLoading === "video" ? "Gerando..." : "3. Gerar Vídeo"}
            </Button>
          ) : (
            <Button variant="secondary" disabled title="Geração de vídeo pausada — use roteiro + storyboard + cenas">
              3. Vídeo (em breve)
            </Button>
          )}
          {script?.hook && (
        ...
        {loading ? (
          ...
          <>
            <PipelineStepper status={project.status} videoEnabled={videoEnabled} />
            ...
            {scenes.length > 0 && (
              <div className="space-y-4">
                <h2 className="text-lg font-semibold">Storyboard ({scenes.length} cenas)</h2>
